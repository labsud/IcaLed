...
* Adafruit_SSD1306 . Adafruit library modified by me to add support for ESP8266 (provided) 
* Adafruit_GFX (pre requis pour Adafruit_SSD1306) egalement adapte support ESP8266 (provided)
* LedControlSPIESP8266, based on LedControlSPI & adapted to support ESP8266. Provided.
* Time : Took from  http://www.pjrc.com/teensy/td_libs_Time.html . Provided.
...
#define NBREVENTS 20       // Number of events stored in array
#define EVENTTEXTSIZE 60   // Size of text description of event    "'Initiation a la decoupe laser'
...
void parseResponse() {
  ...
  while(client.available()){
    ...
    if  (line.startsWith("END:VEVENT")) {
      ...
      i+=1;
      if (i==NBREVENTS) {
        Serial.print ("ALERT : Ical feed contains more than ");
        Serial.print (NBREVENTS);
        Serial.println (" events.");
        }
      }
    if  (eventStarted) { // We're only interrested on DTSTART/END for events.
